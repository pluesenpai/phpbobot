<?php
	...
	function bpp($socket, $channel, $sender, $msg, $infos)
	{
		...
		if($always == true && $is_bpp_on[$channel] == false) {
			return;
			if($infos[1] != "PRIVMSG")
				return;
		}
		...
		$wrong = $db->select(array("bpp"), array("var", "meaning"), array("", ""), array("var"), array("IN"), array("('" . implode("', '", $vars) . "')"), 0, "asc*var");
		...
		if(count($wrong) > 0) {
			...
			if($always == false)
				sendmsg($socket, "{$sender} " . $translations->bot_gettext("bpp-meant") . ": {$msg}", $channel);
			else
