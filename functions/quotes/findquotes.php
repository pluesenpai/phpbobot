	...
	for($i = 1; ($i < $user_num) && ($i < 10); $i++) {
		...
		if(count($result) > 0) {
			foreach($result as $quote)
				sendmsg($socket, IRCColours::BOLD . "#" . $quote["IDQuote"] . IRCColours::Z . " " . IRCColours::UNDERLINE . "(quoted by {$quote["the_sender"]})" . IRCColours::UNDERLINE . ": " . IRCColours::AQUA . "<{$quote["the_poster"]}>" . IRCColours::Z . " " . toUTF8(stripslashes($quote["message"])), $channel);
		} else
